...
const ProjectsSection = ({
  projects,
  loading
}) => loading ? <div className="text-center text-slate-300">Loading projects...</div> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {projects.map((project, index) => <ProjectCard key={project.id} project={project} index={index} />)}
    </div>;
const EducationSection = ({
  educations,
  loadingEdu
}) => loadingEdu ? <div className="text-center text-slate-300">Loading education...</div> : <ul className="space-y-4">
      {educations.map(edu => <li key={edu.id} className="bg-white/10 p-4 rounded-lg">
...
const ExperienceSection = ({
  experiences,
  loadingExp
}) => loadingExp ? <div className="text-center text-slate-300">Loading experience...</div> : <ul className="space-y-4">
      {experiences.map(exp => <li key={exp.id} className="bg-white/10 p-4 rounded-lg">
...
const Index = () => {
  ...
  const {
    educations,
    loading: loadingEdu
  } = useEducations(PUBLIC_USER_ID);
  const {
    experiences,
    loading: loadingExp
  } = useExperiences(PUBLIC_USER_ID);
  ...
  const categories = ['all', ...new Set(projects.flatMap(project => Array.isArray(project.tech_stack) ? project.tech_stack : []))].filter(Boolean);
  ...
  const filterProjects = (searchTerm: string, categories: string[]) => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(project => project.title.toLowerCase().includes(searchTerm.toLowerCase()) || project.description.toLowerCase().includes(searchTerm.toLowerCase()) || Array.isArray(project.tech_stack) && project.tech_stack.some((tech: string) => tech.toLowerCase().includes(searchTerm.toLowerCase())));
    }

    // Filter by categories/skills (multi-select)
    if (categories.length > 0) {
      filtered = filtered.filter(project => Array.isArray(project.tech_stack) && categories.some(category => project.tech_stack.some((tech: string) => tech.toLowerCase() === category.toLowerCase())));
    }
...
– [Recruiter's Name], [Designation]`);
  ...
  return <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 animate-fadeInUp">
      {/* Header Section */}
      <div className="relative overflow-hidden">
        {/* --- Animated Background Layer --- */}
        <div className="pointer-events-none absolute inset-0 z-0">
          {/* Floating SQL Snippets */}
          <div className="absolute left-10 top-10 animate-float-slow text-xs md:text-base font-mono text-white/10 select-none" style={{
          animationDelay: '0s'
        }}>
            SELECT * FROM insights
          </div>
          <div className="absolute right-16 top-32 animate-float-slower text-xs md:text-base font-mono text-blue-200/10 select-none" style={{
          animationDelay: '1.5s'
        }}>
            WHERE sales &gt; 1000
          </div>
          <div className="absolute left-1/3 bottom-24 animate-float-slower text-xs md:text-base font-mono text-purple-200/10 select-none" style={{
          animationDelay: '2.5s'
        }}>
            JOIN customers ON id
          </div>
          <div className="absolute right-1/4 bottom-10 animate-float-slow text-xs md:text-base font-mono text-green-200/10 select-none" style={{
          animationDelay: '3.5s'
        }}>
            GROUP BY region
          </div>
        ...
        <div className="relative container mx-auto px-6 py-16 text-center z-20">
          ...
          <div className="absolute top-4 left-4 flex items-center gap-3">
            {profilePicUrl ? <img key={`${profilePicTimestamp}-${profilePicExtension}`} // Force re-render when timestamp or extension changes
          src={profilePicUrl} alt="Profile" className="w-10 h-10 rounded-full object-cover border-2 border-blue-400" onLoad={() => {
          ...
          <div className="absolute top-4 right-4 flex gap-2">
            {resumeUrl && <button onClick={handleDownloadResume} className="bg-green-600/80 hover:bg-green-700 text-white px-6 py-3 rounded-full transition-all duration-300 flex items-center space-x-2 hover:scale-105 transform animate-scale-in">
                <Download className="w-5 h-5" />
                <span>Resume</span>
              </button>}
            {user ? <Link to="/dashboard" className="bg-white/10 backdrop-blur-md border border-white/20 rounded-full px-6 py-3 text-white hover:bg-white/20 transition-all duration-300 flex items-center space-x-2 hover:scale-105 transform animate-scale-in">
                <User className="w-5 h-5" />
                <span>Dashboard</span>
              </Link> : <Link to="/auth" className="bg-white/10 backdrop-blur-md border border-white/20 rounded-full px-6 py-3 text-white hover:bg-white/20 transition-all duration-300 flex items-center space-x-2 hover:scale-105 transform animate-scale-in">
                <User className="w-5 h-5" />
                <span>Login</span>
              </Link>}
          </div>
          ...
          <p className="text-xl text-slate-300 max-w-2xl mx-auto mb-8 animate-fadeInUp" style={{
          animationDelay: '0.2s'
        }}>
          ...
          {/* Search Bar */}
          <div className="max-w-2xl mx-auto animate-fadeInUp" style={{
          animationDelay: '0.3s'
        }}>
            <div className="relative">
              <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 text-slate-400 w-5 h-5" />
              <input type="text" placeholder="Search projects or skills..." value={searchTerm} onChange={e => handleSearch(e.target.value)} className="w-full pl-12 pr-4 py-3 bg-white/10 backdrop-blur-md border border-white/20 rounded-full text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent transition-all duration-300 focus:scale-105 transform" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-6 pt-8">
        ...
        <div className="mb-8 flex justify-center items-center w-full max-w-2xl mx-auto
            ...
            p-1 h-14 transition-all duration-300 ease-in-out" role="tablist">
          {['Projects', 'Education', 'Experience'].map(tab => <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-1 rounded-full px-6 py-2 text-base font-semibold transition-all duration-300 ease-in-out
                ...
                hover:bg-white/20 hover:shadow-md`} role="tab" aria-selected={activeTab === tab} tabIndex={activeTab === tab ? 0 : -1}>
              {tab}
            </button>)}
        </div>
        ...
        <div className="transition-all duration-300 ease-in-out">
          {/* Projects Tab */}
          {activeTab === 'Projects' && <div id="projects-section">
              {loading ? <div className="text-center py-16">
                  <div className="w-8 h-8 border-2 border-blue-400 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
                  <p className="text-slate-400 text-lg">Loading projects...</p>
                </div> : <>
                  <BubbleFilter categories={categories} selectedCategories={selectedCategories} onCategoryToggle={handleCategoryToggle} onClearAll={handleClearAllFilters} />
                  {/* Featured Projects Section */}
                  {featuredProjects.length > 0 && <div className="mb-8">
                      <h2 className="text-2xl font-bold text-white mb-4">🌟 Featured Projects</h2>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {featuredProjects.map((project, index) => <div key={project.id} className="animate-fadeInUp hover:scale-105 transition-transform duration-300" style={{
              ...
              return <>
                        {nonFeaturedProjects.length > 0 && <div className="mb-8 text-center">
                            <p className="text-slate-300">
                              Showing {currentNonFeaturedProjects.length} of {nonFeaturedProjects.length} projects
                            </p>
                          </div>}
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                          {currentNonFeaturedProjects.map((project, index) => <div key={project.id} className="animate-fadeInUp hover:scale-105 transition-transform duration-300" style={{
                    animationDelay: `${0.1 * index}s`
                  }}>
                              <ProjectCard project={project} index={index} />
                            </div>)}
                        </div>
                        {nonFeaturedProjects.length === 0 && !loading && <div className="text-center py-16">
                        ...
                        {nonFeaturedProjects.length > 0 && <ProjectPagination currentPage={currentPage} totalPages={totalPagesNonFeatured} onPageChange={handlePageChange} />}
                      </>;
            })()}
                </>}
            </div>}
          ...
          {/* Education Tab */}
          {activeTab === 'Education' && <div id="education-section">
              <EducationSection educations={educations} loadingEdu={loadingEdu} />
            </div>}

          {/* Experience Tab */}
          {activeTab === 'Experience' && <div id="experience-section">
      ...
      <div className="bg-gradient-to-r from-blue-900/30 to-purple-900/30 backdrop-blur-sm animate-fadeInUp">
        <div className="container mx-auto px-6 py-16 text-center">
          ...
          <p className="text-slate-300 mb-8 max-w-2xl mx-auto animate-fadeInUp" style={{
          animationDelay: '0.2s'
        }}>
            Interested in collaborating or discussing data analysis opportunities? I'd love to hear from you.
          </p>
          <div className="flex justify-center space-x-6 animate-fadeInUp" style={{
          animationDelay: '0.4s'
        }}>
            <button onClick={handleEmailClick} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-full transition-all duration-300 flex items-center space-x-2 hover:scale-110 transform">
            ...
            <button onClick={handleWhatsAppClick} className="bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-full transition-all duration-300 flex items-center space-x-2 hover:scale-110 transform">
