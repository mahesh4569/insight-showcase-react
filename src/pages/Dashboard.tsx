  ...
  return <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 animate-fadeInUp">
      ...
      <div className="bg-white/10 backdrop-blur-md border-b border-white/20 animate-scale-in">
        <div className="container mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            <button onClick={() => setShowUploadForm(true)} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition-all duration-300 flex items-center space-x-2 hover:scale-105 transform">
              <Plus className="w-4 h-4" />
              <span>Add Project</span>
            </button>
            <button onClick={handleLogout} className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg transition-all duration-300 flex items-center space-x-2 hover:scale-105 transform">
        ...
        <Tabs defaultValue="projects">
          ...
          <TabsContent value="projects">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-6 hover:bg-white/15 transition-all duration-300 transform hover:scale-105 animate-fadeInUp" style={{
              animationDelay: '0.1s'
            }}>
            ...
            <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-6 mb-8 animate-fadeInUp">
              ...
              <div className="flex items-center gap-6">
                ...
                src={profilePicUrl} alt="Profile" className="w-24 h-24 rounded-full object-cover border-4 border-blue-400" onLoad={() => {
                ...
                <FileUploadForm onFileUploaded={url => {
                console.log('Profile picture uploaded:', url);
                // The event listener will handle updating the state
              }} accept="image/*" bucket="profile-pics" label="Upload Profile Picture" />
              </div>
            </div>
            ...
            <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-6 mb-8 animate-fadeInUp">
              ...
              <FileUploadForm onFileUploaded={url => setResumeUrl(url)} accept=".pdf,.doc,.docx" bucket="resumes" label="Resume File (PDF, DOC, DOCX)" />
              {resumeUrl && <div className="mt-4">
                  <a href={resumeUrl} target="_blank" rel="noopener noreferrer" className="text-blue-400 underline">View Uploaded Resume</a>
                </div>}
            </div>

            {/* Projects Table */}
            <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl overflow-hidden animate-fadeInUp" style={{
            animationDelay: '0.4s'
          }}>
              ...
              {loading ? <div className="p-8 text-center">
                ...
                </div> : projects.length === 0 ? <div className="p-8 text-center animate-fadeInUp">
                  <p className="text-slate-400 mb-4">No projects found</p>
                  <button onClick={() => setShowUploadForm(true)} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition-all duration-300 hover:scale-105 transform">
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {projects.map((project, index) => <tr key={project.id} className="border-b border-white/10 hover:bg-white/5 transition-all duration-300 animate-fadeInUp" style={{
                    animationDelay: `${0.1 * (index + 1)}s`
                  }}>
                          <td className="p-4">
                            <div className="flex items-center space-x-3">
                              {project.image_url && <img src={project.image_url} alt={project.title} className="w-12 h-12 rounded-lg object-cover transition-transform duration-300 hover:scale-110" />}
                              <div>
                          ...
                          <td className="p-4">
                            <div className="flex flex-wrap gap-1">
                              {project.tech_stack.slice(0, 3).map((tech, index) => <span key={index} className="px-2 py-1 bg-blue-600/30 text-blue-300 text-xs rounded-full transition-all duration-300 hover:bg-blue-600/50 hover:scale-105">
                                  {tech}
                                </span>)}
                              {project.tech_stack.length > 3 && <span className="px-2 py-1 bg-slate-600/30 text-slate-300 text-xs rounded-full transition-all duration-300 hover:bg-slate-600/50 hover:scale-105">
                          ...
                          <td className="p-4">
                            <div className="flex items-center space-x-2">
                              {project.live_link && <a href={project.live_link} target="_blank" rel="noopener noreferrer" className="p-2 text-slate-400 hover:text-blue-400 transition-all duration-300 hover:scale-125 transform">
                                  <Eye className="w-4 h-4" />
                                </a>}
                              <button onClick={() => handleEditProject(project)} className="p-2 text-slate-400 hover:text-green-400 transition-all duration-300 hover:scale-125 transform">
                                <Edit3 className="w-4 h-4" />
                              </button>
                              <button onClick={() => handleDeleteProject(project.id)} className="p-2 text-slate-400 hover:text-red-400 transition-all duration-300 hover:scale-125 transform">
          ...
          <TabsContent value="education">
            <div className="mb-4 flex justify-between items-center">
              ...
              setShowEduForm(true);
              setEditingEdu(null);
            }} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg">Add Education</button>
            </div>
            {showEduForm && <div className="mb-6">
                <EducationForm initialData={editingEdu} onSave={async data => {
              if (editingEdu) await updateEducation(editingEdu.id, data);else await addEducation(data);
            ...
            <div className="mt-6">
              {loadingEdu ? <div className="text-slate-300">Loading...</div> : <ul className="space-y-4">
                  {educations.map(edu => <li key={edu.id} className="bg-white/10 p-4 rounded-lg flex flex-col md:flex-row md:items-center md:justify-between">
                      <div>
                    ...
                    setEditingEdu(edu);
                    setShowEduForm(true);
                  }} className="px-3 py-1 bg-green-600 text-white rounded">Edit</button>
          ...
          <TabsContent value="experience">
            <div className="mb-4 flex justify-between items-center">
              ...
              setShowExpForm(true);
              setEditingExp(null);
            }} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg">Add Experience</button>
            </div>
            {showExpForm && <div className="mb-6">
                <ExperienceForm initialData={editingExp} onSave={async data => {
              if (editingExp) await updateExperience(editingExp.id, data);else await addExperience(data);
            ...
            <div className="mt-6">
              {loadingExp ? <div className="text-slate-300">Loading...</div> : <ul className="space-y-4">
                  {experiences.map(exp => <li key={exp.id} className="bg-white/10 p-4 rounded-lg flex flex-col md:flex-row md:items-center md:justify-between">
                      <div>
      ...
      {/* Upload Form Modal */}
      {showUploadForm && <div className="animate-fadeInUp">
          <ProjectUploadForm onClose={handleCloseForm} project={editingProject} isEditing={!!editingProject} />
        </div>}
    </div>;
};
